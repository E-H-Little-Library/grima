...
// }}}
	...
	function getHoldingBarcode($barcode){
		$ret = $this->get('/almaws/v1/items?item_barcode={item_barcode}',
			array(
		...
		$mms_id = $xpath->query('//bib_data/mms_id')->item(0)->nodeValue;
		$holding_id = $xpath->query('//holding_data/holding_id')->item(0)->nodeValue;
		$ret2 = $this->getHolding($mms_id, $holding_id);
		return array($mms_id, $holding_id, $ret2);
				...
				*/
				...
				foreach ($this->form->fields as $field) {
					if (isset($_REQUEST[$field->name])) {
                        $this[$field->name] = filter_var($_REQUEST[$field->name], FILTER_SANITIZE_STRING);
						/* sanitize */
					}
				}
			}
...
// }}}
	...
	function loadFromAlmaBarcode($barcode) {
		global $grima;
		$barcodeInfo = $grima->getHoldingBarcode($barcode);
        $this['mms_id'] = $barcodeInfo[0];
        $this['holding_id'] = $barcodeInfo[1];
        $this->xml = $barcodeInfo[2];
	}
...
*/
	...
	function setFirstIndicator($tag, $ind1){
		$xpath = new DomXpath($this->xml);
